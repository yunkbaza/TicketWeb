...
@Component({
  ...
  template: `
    <div *ngIf="isOpen" class="fixed inset-0 z-[600] flex items-center justify-center bg-slate-950/80 backdrop-blur-md p-4 animate-fade-in">
      <div class="bg-white dark:bg-slate-900 w-full max-w-md rounded-3xl shadow-2xl border border-slate-200 dark:border-slate-800 overflow-hidden">
        ...
        <div class="p-6 border-b border-slate-100 dark:border-slate-800 flex justify-between items-center">
          ...
          <button (click)="closeModal.emit()" class="text-slate-400 hover:text-rose-500 transition-colors">
...
export class AuthModalComponent implements OnInit, OnChanges {
  // 🔥 CORREÇÃO: Declaramos o Input que o TypeScript estava a chorar
  @Input({ required: true }) isOpen: boolean = false;
  
  @Input() mode: 'login' | 'register' = 'login';
  
  // O nome do Output deve coincidir com o que o Shell tenta ouvir (closeModal)
  ...
  // 🔥 Deteta quando o 'mode' muda através do clique na Navbar e atualiza o estado
  ...
  submit() {
    ...
    request.subscribe({
      next: () => {
        ...
        this.closeModal.emit(); // Fecha o modal após sucesso
      },
      error: (err) => {
