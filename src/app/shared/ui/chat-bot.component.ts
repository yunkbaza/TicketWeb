...
@Component({
  ...
  template: `
    <div *ngIf="isOpen()" class="fixed bottom-24 right-4 md:right-8 z-[150] w-[90vw] md:w-96 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl shadow-2xl flex flex-col animate-fade-in-up">
      <div class="bg-rose-600 p-4 flex justify-between items-center text-white rounded-t-3xl">
        <div class="flex items-center gap-3">
          <div class="w-10 h-10 bg-white/20 rounded-full flex items-center justify-center shadow-inner border border-white/20">
          ...
          <div>
            <h4 class="font-black text-sm leading-tight tracking-tight">Suporte Baza</h4>
        ...
        <button (click)="isOpen.set(false)" class="text-rose-200 hover:text-white p-1 rounded-md outline-none">
      ...
      <div #chatScroll class="h-80 p-4 overflow-y-auto bg-slate-50 dark:bg-slate-950 flex flex-col gap-4">
      ...
      <div class="p-3 bg-white dark:bg-slate-900 border-t border-slate-100 dark:border-slate-800 flex items-center gap-2 rounded-b-3xl">
        <input [(ngModel)]="input" (keyup.enter)="send()" type="text" placeholder="Dúvidas sobre pagamento?" class="flex-1 bg-slate-100 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-xl px-4 py-3 text-sm text-slate-900 dark:text-white focus:outline-none focus:border-rose-500">
        <button (click)="send()" [disabled]="!input.trim() || isTyping()" class="w-12 h-12 bg-rose-600 text-white rounded-xl flex items-center justify-center hover:bg-rose-700 disabled:opacity-50">
    ...
    <button (click)="isOpen.set(!isOpen())" class="fixed bottom-6 right-4 md:bottom-8 md:right-8 z-50 bg-slate-900 dark:bg-white text-white dark:text-slate-950 p-4 rounded-full shadow-2xl hover:scale-110 transition-transform outline-none">
      <svg *ngIf="!isOpen()" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" class="w-7 h-7"><path stroke-linecap="round" stroke-linejoin="round" d="M7.5 8.25h9m-9 3H12m-9.75 1.51c0 1.6 1.123 2.994 2.707 3.227 1.129.166 2.27.293 3.423.379.35.026.67.21.865.501L12 21l2.755-4.133a1.14 1.14 0 0 1 .865-.501 48.172 48.172 0 0 0 3.423-.379c1.584-.233 2.707-1.626 2.707-3.228V6.741c0-1.602-1.123-2.995-2.707-3.228A48.394 48.394 0 0 0 12 3c-2.392 0-4.744.175-7.043.513C3.373 3.746 2.25 5.14 2.25 6.741v6.018Z" /></svg>
      <svg *ngIf="isOpen()" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2.5" stroke="currentColor" class="w-7 h-7"><path stroke-linecap="round" stroke-linejoin="round" d="M6 18 18 6M6 6l12 12" /></svg>
    </button>
  `
})
export class ChatBotComponent implements AfterViewChecked {
  ...
  input = '';
  messages = signal<{bot: boolean, texto: string}[]>([{ bot: true, texto: 'Olá! Sou o Suporte BazaTicket. Como posso ajudar com sua compra via Stripe?' }]);
  ...
  send() {
    ...
    setTimeout(() => {
      ...
      this.messages.update(m => [...m, { bot: true, texto: 'Nossa integração Stripe garante 100% de segurança criptografada para o seu pagamento.' }]);
    }, 1200);
  }
}