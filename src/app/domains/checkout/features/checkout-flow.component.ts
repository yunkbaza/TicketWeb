      ...
      @if (cart.items().length > 0) {
        <div class="mt-8 pt-8 border-t border-slate-200 dark:border-slate-800 flex flex-col md:flex-row items-center justify-between gap-6">
          ...
          >
            ...
            {{ isProcessing() ? lang.t().checkout.processing : lang.t().checkout.confirm }}
          </button>
        </div>
      }
    </section>
...
export class CheckoutFlowComponent {
  ...
  protected handleCheckout(): void {
    ...
    const payload = { eventId: items[0].event.id, quantity: items[0].quantity };

    this.reservationService.reserveTicket(payload).subscribe({
      next: () => {
        this.isProcessing.set(false);
        this.toast.show(this.lang.t().checkout.success);
        this.cart.clear();
        this.router.navigate(['/']);
      },
      error: (err) => {
        this.isProcessing.set(false);
        this.toast.show(this.lang.t().checkout.error);
